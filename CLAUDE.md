...
- Do not create commits automatically after each task.
- Wait for the user to test the live local instance and explicitly approve the changes before creating any commit.
- After the user approves the changes, create the requested commit with a conventional message.
- Do not push, merge into local `main`, or push to any remote until the user explicitly approves that step as well.
- If the user asks to "push" without mentioning a remote, still treat that as merge/push to local `main`, but only after explicit approval.
